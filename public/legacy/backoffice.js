(function () {
  ...
  const backofficeRoles = {
    ...
    commercial_team: {
      label: "Comercial",
      summary: "Acesso ao Dashboard, To-Do, Comercial, Contactos e Perfil."
    },
    hr_team: {
  ...
  const adminRoles = new Set(["admin", "coordinator", "vice_coordinator"]);

  const roleDisplayGroups = [
    { label: "Coordenação", hint: "Controlo completo", roles: ["coordinator", "vice_coordinator", "admin"] },
    ...
    { label: "Equipas", hint: "Acesso operacional", roles: ["communication_team", "projects_innovation_team", "commercial_team", "hr_team"] }
  ];
  ...
  function isActualAdmin() {
    return adminRoles.has(getActualRole());
  }
  ...
  function syncTeamMemberRoleFromAccess(form) {
    if (!form?.matches(selectors.teamForm) || !form.elements.account_role || !form.elements.role) {
      return;
    }

    const accessRole = backofficeRoles[form.elements.account_role.value]
  ...
  function canManageCommunication(role = getCurrentRole()) {
    return adminRoles.has(role) || ["communication_team", "team_leader_communication"].includes(role);
  }

  function canManageContacts(role = getCurrentRole()) {
    return adminRoles.has(role) || ["communication_team", "team_leader_communication", "commercial_team", "team_leader_commercial"].includes(role);
  }

  function canManageCommercial(role = getCurrentRole()) {
    return adminRoles.has(role) || ["commercial_team", "team_leader_commercial"].includes(role);
  }
  ...
  function canManageHr(role = getCurrentRole()) {
    return adminRoles.has(role) || ["hr_team", "team_leader_hr"].includes(role);
  }
  ...
  function isAdmin() {
    return adminRoles.has(getCurrentRole());
  }
  ...
  function canAccessView(view, role = getCurrentRole()) {
    ...
    if (view === "documents") {
      return adminRoles.has(role) || teamLeaderRoles.has(role);
    }
    ...
    if (view === "commercial") {
      return canManageCommercial(role);
    }
  ...
  function showAuthView() {
    ...
    $(selectors.authBoot).hidden = true;
  ...
  function applyViewAsRole(role) {
    ...
    state.viewAsRole = role && !adminRoles.has(role) && backofficeRoles[role] && !backofficeRoles[role].legacy ? role : "";
    document.body.classList.toggle("is-member", !isAdmin());
    syncViewVisibility();
    renderAll();
  ...
  async function saveUserPermission(profile, nextRole) {
    ...
    const adminCount = state.userProfiles.filter((entry) => adminRoles.has(entry.role)).length;

    if (profile.id === state.user?.id && adminRoles.has(profile.role) && !adminRoles.has(targetRole) && adminCount <= 1) {
  ...
  function compareTeamMembers(left, right) {
    const leftArea = getTeamArea(left);
    const rightArea = getTeamArea(right);
    const leftRole = findProfileForMember(left)?.role || "";
    const rightRole = findProfileForMember(right)?.role || "";

    const directionPriority = (role) => {
      if (role === "coordinator") return 0;
      if (role === "vice_coordinator") return 1;
      if (role === "admin") return 2;
      return 3;
    };
    const teamLeaderRolesByArea = {
      communication: "team_leader_communication",
      projects: "team_leader_projects_innovation",
      commercial: "team_leader_commercial",
      hr: "team_leader_hr",
      general: "team_leader"
    };
    const leadershipPriority = (role, area) => (
      role === teamLeaderRolesByArea[area] ? 0 : 1
    );

    const leftPriority = leftArea === "direction"
      ? directionPriority(leftRole)
      : leadershipPriority(leftRole, leftArea);
    const rightPriority = rightArea === "direction"
      ? directionPriority(rightRole)
      : leadershipPriority(rightRole, rightArea);
  ...
  async function saveMemberForm({ form, statusElement, memberId, allowAdminFields, successMessage }) {
    ...
    if (!payload.name || !payload.role) {
      setStatus(statusElement, "Preenche o nome antes de guardar.", "error");
      return null;
    }
    ...
    try {
      ...
      if (allowAdminFields && linkedProfile && form.elements.account_role) {
        ...
        if (linkedProfile.id === state.user?.id && adminRoles.has(linkedProfile.role) && !adminRoles.has(targetRole) && adminCount <= 1) {
  ...
  function renderProjectMemberOptions() {
    ...
    const previousPositions = new Map(
      Array.from(wrap.querySelectorAll("[data-project-member-id]")).map((card) => [card.dataset.projectMemberId, card.getBoundingClientRect()])
    );
    updateProjectMemberSearchUI();
    wrap.replaceChildren();
    ...
    visibleMembers.forEach((member) => {
      const isFeedbackTarget = state.projectMemberFeedbackId === member.id;
      const card = createElement("article", `bo-project-member-card${selected.has(member.id) ? " is-associated" : ""}${responsible.has(member.id) ? " is-responsible" : ""}${isFeedbackTarget ? " is-selection-feedback" : ""}`);
      card.dataset.projectMemberId = member.id;
      ...
      input.addEventListener("change", () => {
        ...
        state.projectMemberFeedbackId = member.id;
        renderProjectMemberOptions();
      });
    ...
    animateProjectMemberReorder(wrap, previousPositions);
    state.projectMemberFeedbackId = "";
  }

  function animateProjectMemberReorder(wrap, previousPositions) {
    if (!previousPositions.size || window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      return;
    }

    const cards = Array.from(wrap.querySelectorAll("[data-project-member-id]"));
    cards.forEach((card) => {
      const previous = previousPositions.get(card.dataset.projectMemberId);
      if (!previous) return;
      const next = card.getBoundingClientRect();
      const offsetY = previous.top - next.top;
      if (Math.abs(offsetY) < 2) return;
  ...
  function bindEvents() {
    ...
    $(selectors.teamForm).elements.account_role?.addEventListener("change", (event) => {
      syncTeamMemberRoleFromAccess(event.currentTarget.form);
    });
